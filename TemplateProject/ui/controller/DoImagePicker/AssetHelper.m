...
- (void)getGroupList:(void (^)(NSArray *))result
{
    ...
    void (^assetGroupEnumerator)(ALAssetsGroup *, BOOL *) = ^(ALAssetsGroup *group, BOOL *stop)
    {
        ...
        if (group == nil)
        {
            ...
            //相机胶卷置顶
            for(ALAssetsGroup *g in _assetGroups){
                if([[g valueForProperty:ALAssetsGroupPropertyType] integerValue]==ALAssetsGroupSavedPhotos){
                    [_assetGroups removeObject:g];
                    [_assetGroups insertObject:g atIndex:0];
                    break;
                }
            }
